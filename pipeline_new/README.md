...
1. `01_zero_shot_epi.py`
   - Evaluates `cross-encoder/nli-roberta-base` with its NLI head on `data_new/data_epi.csv`.
   ...
   - Writes metrics, subject predictions, and a confusion matrix to `results_new/01_zero_shot_epi`.
...
2. `02_train_test_epi.py`
   ...
   - Writes the validation metrics CSV, confusion matrix, and OOF predictions to `results_new/02_epi`.
...
3. `03_train_daic_pdch.py`, then `03_test_epi.py`
   - Trains on `data_new/data_pdch.csv` and `data_new/data_daic.csv`.
   - Tests on `data_new/data_epi.csv`.
   - Reports five-fold OOF training metrics and the plain EPI cross-cohort test result in `results_new/03_pdch_daic_epi/test`.
...
```bash
python pipeline_new/01_zero_shot_epi.py
python pipeline_new/02_train_test_epi.py
python pipeline_new/03_train_daic_pdch.py
python pipeline_new/03_test_epi.py
```
