# New three-stage pipeline (moderate vs. severe variant)

This is the `_ms` copy of the pipeline: it reads from `data_ms/` and writes to `results_ms/`, so it can be run alongside the main pipeline without overwriting its outputs.
...
1. `01_zero_shot_epi.py`
   - Evaluates `cross-encoder/nli-roberta-base` with its NLI head on `data_ms/data_epi.csv`.
   ...
   - Writes metrics, subject predictions, and a confusion matrix to `results_ms/01_zero_shot_epi`.
...
2. `02_train_test_epi.py`
   ...
   - Writes the validation metrics CSV, confusion matrix, and OOF predictions to `results_ms/02_epi`.
...
3. `03_train_daic_pdch.py`, then `03_test_epi.py`
   - Trains on `data_ms/data_pdch.csv` and `data_ms/data_daic.csv`.
   - Tests on `data_ms/data_epi.csv`.
   - Reports five-fold OOF training metrics and the plain EPI cross-cohort test result in `results_ms/03_pdch_daic_epi/test`.
...
```bash
python pipeline_ms/01_zero_shot_epi.py
python pipeline_ms/02_train_test_epi.py
python pipeline_ms/03_train_daic_pdch.py
python pipeline_ms/03_test_epi.py
```
