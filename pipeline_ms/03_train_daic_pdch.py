...
PDCH_FILE = os.path.join(PROJECT_ROOT, "data_ms/data_pdch.csv")
DAIC_FILE = os.path.join(PROJECT_ROOT, "data_ms/data_daic.csv")
# EPI_FILE moved to 04_predict_epi.py

RESULTS_ROOT = os.path.join(PROJECT_ROOT, "results_ms/03_daic_pdch_epi_2class")
...
pdch_raw = load_csv_dataset(PDCH_FILE, "train")
daic_raw = load_csv_dataset(DAIC_FILE, "train")

for name, ds, path in [
    ("PDCH", pdch_raw, PDCH_FILE),
    ("DAIC", daic_raw, DAIC_FILE),
]:
    if "labels" not in ds.column_names:
        raise ValueError(f"Missing 'labels' column in {name}: {path}")
...
pdch_rows = prep_source_dataset(pdch_raw, "PDCH")
daic_rows = prep_source_dataset(daic_raw, "DAIC")
train_rows = pdch_rows + daic_rows  # Always use both DAIC and PDCH for training
...
# Save class distribution plot
pdch_counts = np.bincount(np.asarray([int(r["labels"]) for r in pdch_rows], dtype=np.int64), minlength=NUM_CLASSES)
daic_counts = np.bincount(np.asarray([int(r["labels"]) for r in daic_rows], dtype=np.int64), minlength=NUM_CLASSES)
class_distribution = {"PDCH": pdch_counts, "DAIC": daic_counts}
data_distribution_path = os.path.join(PLOT_DIR, "class_distribution_pdch_daic.png")
save_class_distribution_plot(class_distribution, data_distribution_path)
print(f"Saved class distribution plot: {data_distribution_path}")
print(f"Training on DAIC + PDCH | n={len(train_classes)} | counts={np.bincount(train_classes, minlength=NUM_CLASSES).tolist()}")
...
# Use a distinct working dir so its best_model path never collides with BEST_MODEL_DIR
...
# Save training summary with all metrics to match 02 format
final_summary = {
    "n": int(len(oof_true)),
    ...
    **{f"fold_{i+1}_macro_f1": float(final_cv_result["fold_scores"][i]) for i in range(FINAL_N_FOLDS)},
    ...
    "learning_rate": 3e-5,  # LEARNING_RATE hardcoded in 03
    "weight_decay": float(WEIGHT_DECAY),
    "freeze_bottom_layers": 4,  # FREEZE_BOTTOM_LAYERS hardcoded in 03
    "dropout_rate": float(DROPOUT_RATE),
    "class_weight_power": float(EFN_WEIGHT_POWER),
    **{f"precision_{CLASS_NAMES[i]}": float(val_precision[i]) for i in range(len(CLASS_NAMES))},
    **{f"recall_{CLASS_NAMES[i]}": float(val_recall[i]) for i in range(len(CLASS_NAMES))},
...
ax.set_title(f"DAIC+PDCH: {FINAL_N_FOLDS}-Fold CV (pooled out-of-fold)")
...
print("\n" + "=" * 70)
print(f"TRAINING COMPLETE: {FINAL_N_FOLDS}-FOLD CV ON DAIC+PDCH")
print("=" * 70)
