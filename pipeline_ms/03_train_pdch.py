...
RESULTS_ROOT = os.path.join(PROJECT_ROOT, "results_ms/03_pdch")
...
# Hardcoded hyperparameters (best from grid search)
...
if "labels" not in pdch_raw.column_names:
    raise ValueError(f"Missing 'labels' column in PDCH: {PDCH_FILE}")
...
train_rows = pdch_rows  # Train on PDCH only
...
print(f"Training on PDCH only | n={len(train_classes)} | counts={np.bincount(train_classes, minlength=NUM_CLASSES).tolist()}")
...
# Train with hardcoded config on PDCH
shutil.rmtree(BEST_MODEL_DIR, ignore_errors=True)
os.makedirs(OUTPUT_DIR, exist_ok=True)
...
ax.set_title(f"PDCH: {FINAL_N_FOLDS}-Fold CV (pooled out-of-fold)")
...
print("\n" + "=" * 70)
print(f"TRAINING COMPLETE: {FINAL_N_FOLDS}-FOLD CV ON PDCH")
print("=" * 70)
